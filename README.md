...
| `/category/{slug}` | Статьи категории, сортировка, пагинация |
| `/article/{slug}` | Страница статьи |
| `/api/search?q=` | Поиск статей по заголовку (JSON) |
| `/api/category/{slug}/articles` | Подгрузка страницы категории (JSON, для AJAX-пагинации) |
...
**Пагинация** — на странице категории статьи разбиты на страницы с кнопками «назад / вперёд» и номерами. Переключение без полной перезагрузки через API.
